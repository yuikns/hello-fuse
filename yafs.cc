...
static int xmp_setchgtime(const char *path, const struct timespec *chgtime) {
  ...
  res = setattrlist(path, &attributes, (void *)chgtime, sizeof(struct timespec),
                    FSOPT_NOFOLLOW);
...
static int xmp_setcrtime(const char *path, const struct timespec *crtime) {
  ...
  res = setattrlist(path, &attributes, (void *)crtime, sizeof(struct timespec),
                    FSOPT_NOFOLLOW);
...
static int xmp_fallocate(const char *path, int mode, off_t offset, off_t length,
  ...
  fstore.fst_flags = 0;
  if (mode & ALLOCATECONTIG) fstore.fst_flags |= F_ALLOCATECONTIG;
  if (mode & ALLOCATEALL) fstore.fst_flags |= F_ALLOCATEALL;
